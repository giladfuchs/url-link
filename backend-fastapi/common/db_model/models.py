...
class LinkModel(DBModel):
    ...
    async def add_or_find_update_single(
        cls,
        add_or_id: str | int,
        body: _payload.LinkPayload,
        **kwargs: Any,
    ) -> table_model.Link:
        if add_or_id == "add" and not cls._get(body, cls.table.alias.key, strip=True):
            cls._set(body, cls.table.alias.key, cls.generate_random_alias())

        cls.validate_link(link=body)
        ...
        cls._get_alias_cache().set(link=link)
        return link
    ...
    async def delete_rows(
        ...
        results = await super().delete_rows(filter_query=filter_query, offset=offset)

        alias_cache = cls._get_alias_cache()
        for obj in results:
            alias_cache.delete(obj.alias)
    ...
    async def fetch_rows_with_totals(
        ...
        **kwargs,
        ...
        since = cls.utcnow() - timedelta(days=days)
...
class VisitModel(DBModel):
    ...
    async def fetch_report(cls, link_id: int, days: int = 30):
        since_date = cls.utcnow().date() - timedelta(days=days - 1)
        sql = """
        WITH base AS (
          SELECT
        ...
        stmt = (
            ...
            .params(link_id=link_id, since=since_date)
        )
...
if __name__ == "__main__":
    ...
    async def main():
        a = await UserModel.fetch_rows(limit=1)
        print(a)
