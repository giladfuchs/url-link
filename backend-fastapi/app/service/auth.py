...
class Auth(BaseUtils):
    ...
    @classmethod
    def _build_google_redirect_uri(cls, request: Request) -> str:
    ...
    def google_login(cls, request: Request):
        ...
        params = {
            ...
            "redirect_uri": cls._build_google_redirect_uri(request),
            "response_type": "code",
    ...
    async def google_callback_issue_token(cls, request: Request) -> Token:
        ...
        if not code:
            cls.error_400("google_oauth_error:no_code")
        if not state or state != request.session.get("oauth_state"):
            cls.error_400("google_oauth_error:bad_state")
        ...
        if not id_token:
            cls.error_400("google_oauth_error:no_id_token")
        ...
        if verify.status_code != 200:
            cls.error_400("google_oauth_error:id_token_verify_failed")
        ...
        if aud != conf.GOOGLE_CLIENT_ID or iss not in (
            ...
            cls.error_400("google_oauth_error:aud_iss_mismatch")
        ...
        if not user.email:
            ...
            if not user["email"]:
                cls.error_400("google_oauth_error:no_email")
