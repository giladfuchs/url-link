...
<br/>
Built with Next.js 16, FastAPI (with background tasks), PostgreSQL, and Google Authentication.
...
NEXT_PUBLIC_EMAIL_CONTACT=contact@email.com
...
```bash
 docker run --name url-link-pg \
   -e POSTGRES_USER=admin -e POSTGRES_PASSWORD=admin \
   -p 5437:5432 \
   -v url_link_pgdata:/var/lib/postgresql/data \
   -e POSTGRES_INITDB_ARGS="--locale-provider=libc --locale=C.UTF-8" \
   -d postgres:17
```
